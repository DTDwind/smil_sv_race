...
import glob
from xvectorLoader import XvectorLoader 
...
class SpeakerNet(nn.Module):
    ...
    def train_network(self, loader):
        ...
        for data, data_label in loader:
            ...
            for inp in data:
                outp      = 1#self.__S__.forward(inp.cuda())
    ...
    def evaluateFromListSave(self, listfilename, print_interval=5000, feat_dir='', test_path='', num_eval=10):
        ...
        print("Computing!");
        xv = XvectorLoader()
        xv.read_scp('vox_Xvector/vox2_dev_x_vector/xvectors_voxceleb1_test/xvector.scp')
        xv.read_ark('vox_Xvector/vox2_dev_x_vector/xvectors_voxceleb1_test/')
        for idx, line in tqdm(enumerate(lines), ascii=True):
            ...
            # print(com_id)
            # exit()
            ref_feat = torch.tensor(numpy.array(xv.get_xvector(ref_id))).cuda()
            com_feat = torch.tensor(numpy.array(xv.get_xvector(com_id))).cuda()
            # if feat_dir == '':
            ...
            dist = F.pairwise_distance(ref_feat.unsqueeze(0), com_feat.unsqueeze(0)).detach().cpu().numpy();
