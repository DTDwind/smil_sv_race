...
from models.classifier_with_score import *
...
class SpeakerNet(nn.Module):
    ...
    def train_network(self, loader):
        ...
        for data, data_label in loader:
            ...
            nloss, prec1 = self.fine_tune_network.forward(feat, label) # resnet score + ori_wav*2 (希望可以用 lstm 處理...) 輸出 暫定sigmoid
    ...
    def evaluateFromListSave(self, listfilename, print_interval=5000, feat_dir='', test_path='', num_eval=10):
        ...
        for idx, line in tqdm(enumerate(lines), ascii=True):
            ...
            feat = torch.stack(feat,dim=1).squeeze()
            score = self.fine_tune_network.forward(feat, eval_mode=True).detach().cpu().numpy();
